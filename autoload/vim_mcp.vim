...
function! s:SearchHelp(query)
  call s:DebugLog('Searching help for: ' . a:query)
  ...
  try
    ...
    try
      ...
      call s:DebugLog('Found exact help match for: ' . a:query)
      return {
        \ 'success': 1,
        \ 'tag': l:tag,
        \ 'file': l:file,
      ...
      call s:DebugLog('Exact match failed, trying fuzzy search')
    endtry
    ...
    try
      ...
      if exists(':cc')
        ...
        call s:DebugLog('Found pattern match via helpgrep')
        return {
          \ 'success': 1,
          \ 'tag': l:tag,
          \ 'file': l:file,
      ...
      call s:DebugLog('Helpgrep also failed: ' . v:exception)
    endtry
    ...
    for l:variation in l:variations
      try
        ...
        call s:DebugLog('Found help with variation: ' . l:variation)
        return {
          \ 'success': 1,
          \ 'tag': l:tag,
          \ 'file': l:file,
    ...
    " No help found
    call s:DebugLog('No help found for: ' . a:query)
    return {
      \ 'success': 0,
      \ 'query': a:query,
    ...
    call s:DebugLog('Error in SearchHelp: ' . v:exception)
    return {
      \ 'success': 0,
      \ 'error': v:exception,
      \ 'query': a:query
  ...
  try
    ...
    elseif has_key(l:message, 'method')
      ...
      elseif l:message.method == 'search_help'
        call s:DebugLog('Searching help for: ' . l:message.params.query)
        let l:response.result = s:SearchHelp(l:message.params.query)
        call s:DebugLog('Help search completed, sending response')
      else
